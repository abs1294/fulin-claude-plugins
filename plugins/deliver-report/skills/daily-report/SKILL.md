...
兩種提醒的節奏不同（使用者明定），這是刻意分開的：

| | 忘了接鏈 | 過去的缺口 |
|---|---|---|
| 頻率 | **每個日報週期一次**——有新的寄送就會再提醒 | **永久只問一次** |
| 為什麼 | 「這次寄完有沒有接鏈」每次都該問 | 你說過不補的日子，不該在下次寄信後又被翻出來 |
| 怎麼記 | 內部狀態記下上次提醒時的寄送快照 | 寫進設定檔的 `schedule.acknowledged_gaps` |

缺口的結案時機是**提醒的下一輪**——那一輪就代表使用者已經回應過了（不管回補、不補、還是沒理），整批日期寫進設定檔永不再問。「問完的下一則就是記錄點」是機械可偵測的時機，不必靠 Claude 記得去登記（那會被漏掉）。

`acknowledged_gaps` 刻意放**設定檔**而不是內部狀態：使用者看得到，想恢復提醒就自己把日期刪掉。實際補寄的日子不會留在清單裡（補了就有寄送紀錄，本來就不算缺口）；超出 `lookback_days` 的舊項目自動裁掉。

還有一條不變的原則：**FAIL-OPEN**——讀檔失敗、解析例外、判斷不確定一律放行。這 hook 影響 session 能不能結束，寧可漏擋絕不卡死。
