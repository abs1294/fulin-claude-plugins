...
9. **易讀性鐵則同樣適用**：本模式產出的是交付文件本體，`document-readability.md` 的八條鐵則（不要兩邊對照／資訊放一起／編號＝執行順序／不用未定義代號／不寫異動紀錄／能自查不丟給讀者／不重複不冗長／交付前全文機械掃描）一律適用，其中鐵則 8 的五項掃描是**交付前必做**。
...
1. **這份報告要證明幾件獨立的事？** 每件事（一個修復、一個工具、一個功能）自成一個區塊，區塊內再挑構件。
   **編號用純序號（1、2、3…），不要加 A-／B-／C- 前綴**（2026-08-25 使用者裁決：問「A-N 是啥」＝沒過關）。多區塊時用區塊標題區隔，不靠字母前綴；交叉引用一律寫「第 N 項」，不寫「A-N」。詳見 `document-readability.md` 鐵則 4。
