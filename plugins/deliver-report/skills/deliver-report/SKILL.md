...
- **兩個例外要用 `htmlBody`**：①使用者明講要表格、或內容確實需要表格才能讀懂；②**信裡有要對方複製貼上的網址或指令**（見下一段，純文字會被 Gmail 改寫成轉址連結）。用了就要自己確認 HTML 簡潔（`<p>`、`<ul>`、`<table>`、`<pre>` 為主，不要塞 CSS）。
...
2. **可貼的內容包在 `<pre>` 裡**，其餘照舊用 `<p>`、`<br>`、`<b>`，一樣不要塞 CSS。`<pre>` 除了保住網址，也順便保住換行與縮排——純文字路徑另一個坑是**多行設定會被擠成同一行**（`}location = /connect.php`），對方根本貼不了。
