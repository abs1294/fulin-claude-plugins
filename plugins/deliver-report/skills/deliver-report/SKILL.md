---
name: deliver-report
description: 把本次交付的產出檔案寫成一段口語、務實的交付訊息（fulin 慣用格式與語氣）。當使用者說「交付」、「寫交付訊息」、「產出交付信」、「給使用者的交付」、「deliver」、「交付報告」、「這批檔案要交出去」、「幫我寫給客戶/工程端的說明」、「寫個 handoff」、「整理成交付訊息」、「回覆對方的提問並附報告」、「這批要給好幾組人/多個單位」時觸發。內容依當次產出而定，語氣固定。動筆前先定四件事：檔案是主體還是佐證、有沒有對方的問題要回、單一收件人還是多類分眾、內文密度。把技術詞翻成人話，非正式官腔。
---

# deliver-report — 交付訊息產生器

> 使用者常要把一批產出（報告、Excel、log、設定檔…）交付給他的收件人。**語氣固定，形狀依當次情境而定。** 這個 skill 只負責把當次產出寫成那段訊息。
>
...
**這四件事各自獨立決定、可自由組合。不要去找自己屬於哪一型——沒有型，只有這四個決定。** 決定完了，信的形狀自然就出來了。
...
統一判準，不分情境：**寫下每一句前先問「這句不寫，對方會不會做錯事、或對現況產生錯誤認知」**——都不會才刪。
...
> **兩者同時成立時（檔案是主體、對方又有提問）以「決定一＝主體」為準**：題目當骨架，但逐檔說明與帶數字的結論要留，不要壓成一句話。判斷依據是**這批檔案對方要拿去做什麼**——要拿去審、去簽核、去排施作順序的，就得寫清楚。（這個組合沒有範例可抄，照這裡的規則自己長。）
...
> 反例三：「後端調整若只套用一半，登入憑證安全屬性會靜默失效」——**即使該調整由我方執行，也要進信**。理由：交付之後改設定的人可能是對方、代管商或下一任工程師；「靜默」意味著出事時沒有任何訊息會提醒任何人。**凡是安全性失效，一律進信，不因「是我方的施作紀律」而降級。**
>
> **有疑慮時一律進信。** 寫長一句被退信可以重寫；警示被靜默丟掉，是對方受損後才會發現，而且歸因會是「你們交付時沒講」。
...
> 示範重點：組織軸是**對方的題號**（檔案退到結尾）、**題目標籤照抄對方用詞**（「Nginx HTTPS Reverse Proxy」沒改寫）、**第 2 題前提校正**（對方問測試環境建置與需要什麼協助，我方先講「已建好、不需你給資料」再講「但需要另一批資料才能收斂工作天」）、每題最多三句。

```
Hi B,

關於B 論壇論壇（b-forum.example.com/forum.php）啟用 HTTPS，來信的三個問題我們評估完了。我們已在 我方 這邊用同版本的論壇軟體（Discuz! X3.2 繁中 UTF8，R2016xxxx）建好一座對等測試站，把整套架構實際跑過一遍，請詳以下三題回覆：

第 1 題（Nginx HTTPS Reverse Proxy 方案的可行性）

可行，我方 認同以此架構作為執行方向，我們已實際部署驗證通過。分階段導入的作法我們同意，建議切成三階段，細節在盤點報告。
...
- **密度差異來自決定組合，不是來自「選了不同的型」**：範例一、二每檔寫兩三句並展開子項；範例四每題最多三句、附件一句話帶過。同一個 skill 兩種密度是正常的。
