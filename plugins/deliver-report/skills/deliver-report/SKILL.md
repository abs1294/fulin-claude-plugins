...
>
> **三個鐵則**：①**稱謂行一律用半形逗號結尾**（`Dear Jen,`／`Hi 佳宣,`），**收件人沒指定就寫 `Hi,`，永遠不留 `<收件人>` 這種佔位符**（忘了改會很難看）。②**預設讀者不懂技術**——寫給主管/客戶/業務看的，用白話，術語只留在給工程端的那一檔。③**這是對外說明，嚴禁任何內部術語/流程字眼**（紅藍對抗、QA、code-review、adopt/bump/publish、skill、plugin、session、AI…）——只講交付物與結論，不講我方怎麼做出來的。
...
> 這裡的「訊息」指的是**內容形態**（文字，不是 DOCX），不是**交付管道**。同一段文字可以貼在對話裡給使用者自己送，也可以照第三步之三建成 Gmail 草稿——**建草稿仍然只到草稿為止，寄不寄由使用者按**。

**文件易讀性鐵則：任何要交給別人看的產出都適用**（交付訊息本身、施作說明書、驗收清單、提案、評估報告、交接文件）——一律先讀 `../../references/document-readability.md`：十四條鐵則＋交付前必做的九項機械掃描。**不要因為「這份只是內部參考」「這份不用照著做」就放寬**（2026-08-25 使用者裁決：「不只吧，所有的文件你都有可能發生這種低級錯誤啊」）。該檔每一條都是使用者當面指出過 2~4 次的實案，**不是理論**。
...
2. **收件人**：若使用者這次有講收件人（例如「給 A」），就寫 `Hi A,`。**沒講就直接寫 `Hi,`**——絕不輸出 `Hi <收件人>,`、`Hi ___,` 這類佔位符（忘了改會很難看）。乾淨的 `Hi,` 本身就成立，寧可留白也不留佔位符。
   **稱謂行的逗號一律用半形 `,`**（`Dear Jen,`／`Hi 酩翔,`／`Hi A & B,`），不用全形「，」——這是使用者的固定偏好。對方是外商窗口或英文名時用 `Dear`，內部/熟識同事用 `Hi`。
...
>
> 形狀裡尖括號 `<…>` 只是給你看的填空提示，**最終輸出絕不能留任何尖括號佔位符**。收件人沒指定就只寫 `Hi,`（半形逗號）。
...
- **稱呼對方一律用「您們」（單人用「您」），禁止公文式敬稱**：不寫「貴司」「貴公司」「貴中心」「貴單位」「貴部門」「貴部」「貴院」「貴局」「貴處」「貴校」「貴廠」「貴組」「貴方」「您司」，也不自稱「敝司」「敝公司」「敝中心」「本司」「本中心」。理由：這類敬稱是公文腔，讀起來有距離感；交付訊息要像人講話，「您們」既禮貌又親近。需要指單位時就直接寫單位名稱（「資訊室」「桃園廠」），不加「貴」字。
...
## 第三步之三：能建草稿就別叫使用者複製貼上

**判準：這封信有明確的收件人與信件管道（Gmail），就直接用 Gmail 工具建成草稿，不要只把文字丟在對話裡叫使用者自己貼。**

最常見的觸發是**使用者請你去看信**（「你看一下這兩封信」「他們來信要議價」「幫我回這封」）——**看完信就順手把回覆建成草稿**，讓使用者只需按送出，省去複製、貼上、重排格式、補收件人這四道工。使用者沒說「建草稿」也照做；他叫你看信，通常就是要回信。

**建草稿的四個要點**：

1. **掛回原信串**：回覆既有信件時帶 `replyToMessageId`（用該封信的 message id，不是 thread id），Gmail 才會把草稿接在原討論串下、主旨自動帶 `RE:`。新信才不帶。
2. **收件人照原信的回覆規則**：原信寫「回覆請使用本封郵件進行回覆」就回原寄件位址；原信 To／Cc 裡的內部關係人（採購承辦、需求端窗口）判斷是否保留，**多帶一個需求端窗口通常對使用者有利**（讓他知道採購在動作），但要在對話裡告訴使用者你帶了誰、讓他決定要不要拿掉。
3. **一次只建一封、一案一封**：兩個案子（不同案號、不同公司）就建兩封各自掛在自己的信串，不要合併成一封群發。
   **每封動筆前重新核對「這封信是哪個案子、對方問的是哪幾件事」**，只用那個案子的事實——同一輪對話裡處理過的另一案資訊不得沿用。兩案的報價、時程、對方窗口在腦中很容易串，而寄錯案的資訊給對方是收不回來的。
4. **建完在對話裡摘要**：列出「收件人／副本／立場」一小張表，並點出兩封信之間**刻意的差異**（例如同一句話對 A 客戶能寫、對 B 客戶不能寫的理由）。使用者要能不開 Gmail 就知道你寫了什麼。

**格式：用 Gmail 原生純文字，非必要不要送 HTML。**

- **預設只給 `body`（純文字），不要給 `htmlBody`**。Gmail 會用它原生的富文本編輯器呈現，字體、行距、簽名檔都跟使用者平常寄的信一致；塞 HTML 進去反而會出現字體不一致、行距怪異、與簽名檔衝突。
- **排版靠空行與破折號，不要靠標籤**：條列用「項目：值」一行一條（前面不加 `-`／`*` 符號也可以，Gmail 純文字看起來更乾淨）；段落之間空一行。**不要用 Markdown 語法**（`**粗體**`、`##`、`|表格|`）——那在 Gmail 裡會原樣顯示成星號與井號，很難看。
- **兩個例外要用 `htmlBody`**：①使用者明講要表格、或內容確實需要表格才能讀懂；②**信裡有要對方複製貼上的網址或指令**（見下一段，純文字會被 Gmail 改寫成轉址連結）。

> **用 `htmlBody` 時只准用這組標籤**（白名單，其餘一律不准）：
> `<br>`　`<hr>`　`<b>`　`<i>`　`<u>`　`<ul>` `<ol>` `<li>`　`<a>`　`<pre>`　`<table>` `<tr>` `<td>` `<th>`
>
> （`<strong>`／`<em>`／`<code>`／`<thead>`／`<tbody>` 是上面幾個的等價寫法，hook 一併放行，但寫的時候用上面那組就好。）
>
> **換行一律用 `<br>`，禁止用 `<p>`。** 段落之間空一行就是 `<br><br>`。
> 外層可以包一個 `<div dir="ltr">`，那是 Gmail 自己的結構。
>
> **為什麼禁 `<p>`**（兩個理由，第二個才是主因）：
> 1. `<p>` 本身有段落間距，原始碼裡再留空行會疊成雙倍行距。
> 2. **更重要：草稿是要讓使用者在 Gmail 富文本編輯器裡微調的。** 塞 `<p class=…>`
>    這種結構進去，使用者改一個字就可能整段跑版——而寄不寄、怎麼微調本來就是他的事。
>    `<div dir="ltr">` ＋ `<br>` **正是 Gmail 編輯器自己產生的結構**，所以草稿在編輯器裡的
>    行為會跟他手打的信完全一致。
>
> **禁止**：`class=`、`style=`、`<span>`、`<div>`（外層那一個除外）、任何 CSS。
>
> 實測（2026-09-16 建草稿讀回比對位元組）：`<br>`／`<b>`／`<i>`／`<ul>`／`<li>`／`<table>`
> 送進去讀回來**一個位元組都沒變**；而 `<p>` 之間留空行會讀回 `</p>\r\n\r\n<p>`，
> 也就是收件者看到的雙倍行距。

**寫法範例**（含指令、清單、表格的完整一封）：

```html
<div dir="ltr">Hi 暉東,<br><br>正式站的 HTTPS 已經上線，18 項裡 17 項通過。<br><br>請把步驟 7 這三行的 <b>308 改成 307</b>，其餘不動：<br><pre>location = /api.php     { return 307 https://www.example.com$request_uri; }
location = /connect.php { return 307 https://www.example.com$request_uri; }</pre><br>改完確認：<br><ul><li>回應要有 Location 這一行</li><li>而且指向加密版網址</li></ul><br>Best Regards,<br>甫軒</div>
```

重點：**標籤之間不留空行**（要空一行就寫 `<br><br>`），但 **`<pre>` 裡面的換行照舊保留**——
多行設定就是靠它才貼得出來，實測兩種寫法下 `<pre>` 內的換行都不會被動。
- **不要在草稿裡加簽名檔**——Gmail 會自動附上使用者設定的簽名，重複加會出現兩份。結尾寫到「Best Regards,」加名字即可，或連名字都不寫讓簽名檔接手（看使用者習慣）。

**信裡有網址／指令／設定內容時，一律改走 `htmlBody`——純文字 `body` 會被 Gmail 竄改。**

Gmail 建草稿時會把內文裡的網址**自動改寫成自家轉址連結**，長這樣：

```
原本寫的：  return 307 https://www.nuvoton-mcu.com$request_uri;
對方看到的：return 307 https://www.google.com/url?q=https://www.nuvoton-mcu.com&source=gmail&ust=1789569257762000&sa=E$request_uri
```

**後果是對方照貼就貼到錯的位址**——設定檔的轉址目標變成 Google、curl 指令打到 Google。信裡只要有「請照這段貼上去」的內容（nginx／Apache 設定、curl／wget 指令、API endpoint、資料庫連線字串、檔案下載連結），就會中。

**實測結論**（2026-09-15 對 Gmail 建草稿實打驗證，非推論）：

| 寫法 | 結果 |
|------|------|
| `body` 純文字，網址直接寫 | ❌ 被改寫 |
| `body` 純文字，去掉 `https://` 只留 `www.x.com` | ❌ 照樣被改寫（還自動補上 `http://`） |
| `body` 純文字，網址包 `[ ]` 或 `< >` | ❌ 被改寫 |
| `body` 純文字，整段縮排四格當程式碼區塊 | ❌ 被改寫 |
| `body` 純文字，塞零寬空格斷開 | ❌ 被改寫，而且把零寬字元一起吃進網址裡 |
| **`htmlBody`，網址直接寫**（含 `<pre>`、`<code>`、`<a>`） | ✅ **顯示文字原封不動**，只有 `href` 被改寫 |

**所以做法是**：

1. **這封信只要含一個以上「要對方複製貼上」的網址或指令，就給 `htmlBody`**——這是前面「預設只給純文字」那條規則的明確例外，不必再找使用者確認。
2. **可貼的內容包在 `<pre>` 裡**，其餘照上面的白名單用 `<br>`、`<b>`，一樣不要塞 CSS。`<pre>` 除了保住網址，也順便保住換行與縮排——純文字路徑另一個坑是**多行設定會被擠成同一行**（`}location = /connect.php`），對方根本貼不了。
3. **`body` 還是要給**（同一份內容的純文字版），當作不支援 HTML 的信箱的退路；那一份裡的網址會被改寫，所以**純文字版把可貼內容換成一句「完整內容見報告第 N 節」**，不要留半殘的指令。
4. **建完一定要 `get_draft` 讀回來看**，用 `messageFormat: FULL_CONTENT` 檢查 `htmlBody` 裡**標籤之間的顯示文字**是不是原本寫的網址（`href` 被改寫是正常的、不用管）。**掃到 `google.com/url?q=` 出現在顯示文字裡就是壞了**，重建。

**沒辦法用 HTML、或內容太長的時候**（例如整份設定檔），才退成「信裡不貼、指到報告第 N 節」，並且**在信裡明講為什麼**（「信件系統會自動改寫網址，貼在信裡反而會貼到錯的位址，完整內容放在報告第五節」）——讓對方知道要去翻報告，而不是以為你漏給了。**不要默默把網址整個拿掉、也不要代換成 `[站台網址]` 這種佔位符**：對方看不懂那要填什麼（2026-09-15 使用者當面退過這個做法）。

**建完務必說明「已建草稿、未寄出」**——寄信是對外動作，一律讓使用者自己按送出。

## 第四步：交出草稿，讓使用者微調

把整段訊息**直接輸出成一塊可整段複製的文字**（不要拆段落夾註解）。輸出後可補一句「收件人/檔名/版本要調再跟我說」。

**已依第三步之三建成 Gmail 草稿時**，對話裡就不必再貼一次全文——改成給那張「收件人／副本／立場」摘要表，加上要使用者拍板的幾點（副本帶了誰、有沒有沒開的附件、兩封信的刻意差異）。全文已經在草稿裡，重貼一次只是洗版。
