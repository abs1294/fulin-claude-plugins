...
> **本 plugin 補充**：對外送出的問卷屬交付物——語氣與術語密度照同 plugin `deliver-report` 的鐵則②③（預設讀者不懂技術、嚴禁內部流程字眼）；易讀性照 `../../references/document-readability.md` 十四條鐵則，交付前跑九項機械掃描。問卷寫好後若要配一封寄送信，走 `deliver-report`。
