...
description: 把「已做完的測試結果」產成一份給 PM／客戶簽核轉發的測試報告 DOCX 檔。當使用者說「測試報告」、「修復測試報告」、「驗收報告」、「給 PM 的報告」、「report 要 Word/DOCX 檔」、「把測試結果寫成報告」、「產一份測試證明文件」時觸發。**前提是測試已做完、證據（截圖／紀錄）在手**——測試還沒跑、要設計或執行測試的，用 qa 系 skill；本 skill 只把既有測試結果文件化，不憑描述寫報告。不套固定目錄：先做組裝決定（要證明幾件事、對方拿去做什麼、修復還是新功能、有沒有動正式環境、對方之前看過什麼），再從段落構件庫按需取用。含十條鐵則（禁字清單／不寫 meta 句／讀者不懂技術／實機截圖為主證據／缺陷走交付揭露不進報告／只寫最終狀態不寫我方作業時序／不寫我方修了什麼 bug／宣稱須逐項對得上實際檢核／易讀性鐵則適用／每個情境都要有證明）。測試結果採「總覽表＋逐項展開」版面（證據放在該情境段落底下，不拆成表一節圖一節），總表與逐項的對應由 scripts/report_gate.py 三道機械閘強制，閘沒過不產檔、不交付。**產出的是檔案不是訊息**——報告定稿後若還要寫一段交付訊息把它交出去，改用 deliver-report skill。
---
...
8. **總覽表的宣稱必須逐項對得上實際檢核**（使用者裁決 2026-08-13）：寫「N 項條件皆可篩選」
...
9. **易讀性鐵則同樣適用**：本模式產出的是交付文件本體，`../../references/document-readability.md` 的十四條鐵則（不要兩邊對照／資訊放一起／編號＝執行順序／不用未定義代號／不寫異動紀錄／能自查不丟給讀者／不重複不冗長／交付前全文機械掃描／視覺標記成套一致／能腳本化就不叫人手動／改文件前先讀實際樣貌／做完的事寫成做完／內部推導不進對外文件／稱呼對方用「您們」不用公文式敬稱）一律適用，其中**該文件**鐵則 8 的九項掃描是**交付前必做**（第 9 項公文式敬稱是提醒、不擋）。

10. **每個進報告的情境都必須指得出證明，形式不限圖片**（2026-09-14 使用者裁定：「一定要有證明，不管證明是不是圖片」）。
    可接受形式六種：**實機截圖**（有畫面入口的功能，主證據，見鐵則 4）、**資料庫查詢結果**（實值）、**系統傳輸紀錄／送出參數**、**執行紀錄／log 行**、**寄出的信件實體**（背景寄信等無畫面機制）、**匯出檔案**。
    **不算證據**：判定欄的「通過」、實測欄填「已驗證」「測試通過」「正常」「符合預期」這類無指涉詞、「見上」「同上」。
    補不出證據的情境**從報告移除，走交付揭露**（鐵則 5），不得留在報告裡用空話撐過去。
    > 這條與〔測試結果〕的版面改動是同一件事的兩面：版面保證「證據放在情境底下」，本條保證「底下真的有東西」。兩者都由〈機械閘〉強制，不靠自律。

## 組裝前先做：證據盤點

動筆前把要引用的證據**實際開檔核對一遍，別憑對話記憶**：截圖檔真的存在且內容正確、資料庫實值／傳輸參數有留存紀錄可引用、每個要寫進逐項驗證段落的「實測結果」都指得出對應證據。缺的分兩種：**已驗證過的行為只是畫面／紀錄沒留存**——重新擷取即可（重開畫面截圖、重撈紀錄），仍屬文件化。「已驗證過」的認定不憑宣稱：要指得出當時留下的其他佐證（測試紀錄、執行輸出、既有報告草稿之一）；指不出來、或重擷結果與當時佐證不一致，一律當測試沒做完處理。**需要重新執行測試情境才能產生的證據**——那是測試沒做完，退回 qa 系 skill 補跑，不在本模式內做。兩者都補不了就從報告中省略該情境、並在「交付前的揭露」對使用者如實標註——不可憑印象把「實測結果」欄填滿（那是換條路徑違反「不憑描述寫報告」）。**盤點結果直接寫成生成腳本的 `CASES`**，由 `validate_cases()` 機械覆核（見〈機械閘〉）——人工盤點會漏，閘不會。
...
2. **對方拿報告去做什麼？** 簽核放行 → 結論與測試結果總覽表前置；理解問題始末 → 白話對照前置；照著操作 → 執行程序構件必入且靠後放。
...
### 〔測試結果〕——總表＋逐項展開（**版面固定，不可拆成「表一節、圖一節」**）

**舊版面（測試表獨立一節、實測畫面獨立一節）已廢除**（2026-09-14 使用者裁定）。舊做法把「情境」與「它的證據」放在文件兩處，中間只靠圖說那句「對應第 N 項」牽線——單向、無約束，於是「情境 7 沒有任何圖」「圖 9 對不到任何情境」在版面上看不出來，表是滿的、圖也是滿的，只有讀者交叉比對才會發現。**分兩節是對不上的成因，不是表現。**

改成兩者用途分家、內容不分家：

```
〔測試結果總覽〕   ← 簽核者看這張就夠：全過了沒
| 編號 | 測試情境 | 判定 |

〔逐項驗證〕       ← 要細看的人往下讀；每項的證據就在該項底下
### 1　供應商送件後 SAP 收到最新銀行帳號
    預期：…／實測：…
    [圖 1-1]（圖說）  [圖 1-2]（圖說）
### 2　…
```
...
1. **總表只有三欄**（編號／測試情境／判定）。預期、實測、證據全部在逐項段落——總表塞五欄就回到「讀者兩邊對照」，正是 `../../references/document-readability.md` 鐵則 1 禁的事。
2. **逐項標題＝`N　〈測試情境名〉`**，情境名與總表**逐字相同**（不是意思相同）。
3. **圖號用 `N-M`**（情境號－該情境內序號），不用全篇流水號。圖號自帶情境號，孤兒圖與缺圖在版面上直接露餡。

**總表與逐項的一致性不靠自律，靠機械閘**——見下節〈機械閘〉。

#### 總覽表欄位
| 欄位 | 內容 |
|---|---|
| 編號 | 純序號（1、2、3…），逐項標題與圖號都引用它；不加字母前綴 |
| 測試情境 | 一句白話，含資料形態（「材料類別僅有空白明細＋共通有負責人」） |
| 判定 | 通過／不通過／未驗證 |

#### 逐項驗證段落的內容
- **預期結果**：可觀察行為。
- **實測結果**：**值層級證據**——資料庫實值、送出參數逐欄比對；不是「畫面有東西」「回 200」。**不得填「已驗證」「測試通過」「正常」這類無指涉詞**（那是判定不是證據，機械閘會擋）。
- **證據**：至少一筆，形式不限（見鐵則 10）。

證據強度（有就寫，沒有不硬湊——但「一項證據都沒有」不成立，見鐵則 10）：
- **前後對照**：「修正前重現：資料庫正確、SAP 收到舊值」。
- **變異測試**是最強的即時性證明（改值→重送→參數即時反映→還原），有做必列一項。
...
- 圖說格式：`圖 N-M　〈修正前／修正後／系統畫面〉：〈這張證明了什麼〉`。圖已在該情境段落底下，**不必再寫「對應第 N 項」**——那是舊版面的補丁。圖說寫這張證明什麼，不重述情境名。
...
## 機械閘（`scripts/report_gate.py`）——總表與逐項 100% 對應的唯一保證

> **這節不是建議，是強制**。「總表每一列都對得上逐項標題」不靠人工核對，
> 也不靠產出後掃描 DOCX——**掃描抓得到「我寫錯」，抓不到「我根本沒寫」**，
> 而「有漏」正是後者。唯一有效的做法是讓兩節共用同一份資料源。

### 閘的形狀：三道，由強到弱

**閘一（結構性，最強）：情境只准定義一次。**
生成腳本裡所有情境集中在一個 `CASES` list，總表與逐項段落都由**同一個 list** 渲染：

```python
CASES = [
    {
        "id": 1,
        "title": "供應商送件後 SAP 收到最新銀行帳號",
        "expect": "SAP 端帳號＝畫面最新值",
        "actual": "資料庫 0012345 → SAP 傳輸參數 0012345，逐欄一致",
        "result": "通過",                      # 通過／不通過／未驗證
        "evidence": [
            {"type": "image", "path": "_work/shots/01_form.png",
             "caption": "畫面送出值：供應商端已存檔的帳號"},
            {"type": "payload", "content": "BANKN=0012345 / KOINH=某某公司",
             "caption": "送出參數：與資料庫同值，證明非舊值"},
        ],
    },
]
```

`evidence.type` 六選一：`image`／`db`／`payload`／`log`／`mail`／`file`（對應鐵則 10 的六種形式）。
`image` 要 `path`，其餘要 `content`（可引用的實際內容）；**每筆都要 `caption`**。

渲染一律經由閘提供的函式取字，**不得各自寫死**：

```python
for seq, title, result in summary_rows(CASES):   # 總表
    ...
for c in CASES:                                   # 逐項
    doc.add_heading(detail_title(c), level=3)     # 「N　情境名」
    for j, ev in enumerate(c["evidence"]):
        cap = "圖 {}　{}".format(figure_no(c, j), ev["caption"])
```

總表的 `title` 與逐項標題的 `title` 是**同一個變數**——字串不可能歧異，因為只有一份。
對不上在結構上不可能發生，不是「掃過了沒問題」。

**閘二（產出前）：`validate_cases(CASES, base_dir=…)`**
擋資料源本身殘缺，不通過 `raise GateError`（繼承 `SystemExit`）、**不產出任何檔案**。擋這些：
編號非 1..N 連續／情境名重複或為空／缺必要欄位／實測欄是無指涉詞／判定值非法／
**情境沒有任何證據**／證據缺 `caption`／非圖片證據缺 `content`／截圖檔不存在／截圖 < 20 KB（疑似空白頁）。

**閘三（產出後）：`verify_docx(out_path, CASES)`**
用**不同判準**反向讀產出的 DOCX 對帳——不重用渲染邏輯，從 docx 抽總表第一欄、抽 Heading 3 段落、數 `inline_shapes`，再與 CASES 三方比對；最後用 **Word COM 實開**。
理由是全域 CLAUDE.md 驗證層級紀律：施作與驗證共用同一份判準時，驗證器對施作器的盲區完全同構；且 `python-docx` 開得了 ≠ Word 開得了（交付檔案格式紀律）。
擋這些：總表列數 ≠ 情境數／逐項段落數 ≠ 情境數／某編號只在一邊出現／**同編號的情境名兩邊不一致**／產出內容與 CASES 不符／文件圖片數 ≠ `image` 證據數／Word 開不起來。
...
sys.path.insert(0, r"<此 skill 目錄>/scripts")
from report_gate import (validate_cases, verify_docx,
                         summary_rows, detail_title, figure_no, evidence_label)

CASES = [...]
validate_cases(CASES, base_dir=Path(__file__).parent)   # 閘二：沒過就不往下走
...渲染（只經由 summary_rows / detail_title / figure_no 取字）...
doc.save(out)
verify_docx(out, CASES)                                  # 閘三：沒過不得交付
```

`<此 skill 目錄>` 用 `${CLAUDE_PLUGIN_ROOT}/skills/test-report-docx/scripts` 解析；
拿不到該環境變數時寫絕對路徑，**不要把 `report_gate.py` 複製進 scratchpad**——
複製一份等於判準分叉，之後改閘不會回溯到舊腳本。

### 閘本身的自測

改過 `report_gate.py` 就跑一次，確認閘還擋得住（8 種違規案例）：

```
python scripts/report_gate.py --selftest
```

> **不得停用或放寬閘來讓報告產得出來**。閘擋下來代表證據真的缺——
> 該做的是補證據，或把該情境移出報告走交付揭露（鐵則 5、鐵則 10），
> 不是調閘。`strict_image=False` 只在「證據檔在另一台機器、本機僅組版」時使用，
> 且必須在交付揭露裡講明「截圖存在性未經本機驗證」。
