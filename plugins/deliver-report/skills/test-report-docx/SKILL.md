...
9. **易讀性鐵則同樣適用**：本模式產出的是交付文件本體，`../../references/document-readability.md` 的十三條鐵則（不要兩邊對照／資訊放一起／編號＝執行順序／不用未定義代號／不寫異動紀錄／能自查不丟給讀者／不重複不冗長／交付前全文機械掃描／視覺標記成套一致／能腳本化就不叫人手動／改文件前先讀實際樣貌／做完的事寫成做完／內部推導不進對外文件）一律適用，其中**該文件**鐵則 8 的五項掃描是**交付前必做**。
