...
- **`send_common.py` 在 cp950 主控台印字會 `UnicodeEncodeError` 中斷寄送**。`_run_gate` 轉印各閘腳本的輸出時，`✓` `✗` `⚠` 等字元在繁中 Windows 的預設編碼下編不出來；**輸出被導向（pipe／檔案）時 `errors=strict`，直接拋例外**——連完全乾淨、零命中的日報都會掛在「印字」而不是「檢查」。
  - 根因：daily-report 的**六支閘腳本各自都有**把 stdout 轉 UTF-8 的處理，只有 `send_common.py` 漏了——而它正是轉印那些輸出的人。
  - 修法照既有慣例（同樣的 `reconfigure(encoding="utf-8")` 條件式），另加第二道保險 `errors="replace"`：轉換失敗或環境仍編不出時降級為 `?`，**寄送流程不因印字失敗而中斷**。
  - 驗證（實跑）：修復前 `assert_content_clean()` 對乾淨日報在 `PYTHONIOENCODING=cp950` 下拋 `UnicodeEncodeError`；修復後同一條指令 exit 0、`✓` 正常印出。回歸四情境於 cp950 下全對（乾淨／敬稱／成本中心不中斷、真禁字仍中止 code=3），預設編碼下亦正常，`content_guard --selftest` 綠。
  - **範圍修正紀錄**：初判以為 6 支閘腳本都缺這個保護，插入後才發現它們本來就有（我的插入會被既有程式碼覆蓋、等於無效碼），已全部還原，最終只改 `send_common.py` 一處。
