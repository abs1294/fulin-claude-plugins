...
遇到表裡沒有的模型（通常是新版上線）時：
1. 暫時用同系列版本最接近的單價頂著算（例如沒見過的 `opus-6` 先用 Opus 5.5、已退役的 `opus-3` 先用 Opus 4；前後兩個版本一樣近時取較新的那個），金額前面加 **`~`**，表示這是暫估。
...
官方頁也查不到的模型，6 小時內不會再重查，金額維持 `~`。每次抓取最多 45 秒，逾時記下失敗原因後結束；同一時間最多只有一支背景程序在抓。完全找不到同系列的模型不計價（算 0），也會標 `~`。
