...
| `cron-tracker.js` | PostToolUse (CronCreate|CronDelete|ScheduleWakeup) | crons（排程狀態，固定佔 agents/skills 欄底兩行；一次性排程逾時自動視為已觸發剔除，循環排程顯示至刪除） |
...
前置依賴：
- **Node.js** — statusline.js 與 7 支 tracker hooks 都是 node 腳本（缺 node 整條 status line 空白）。
- **git CLI** — repo/branch/變更行數那列用；缺 git 該列留空、其餘正常。
- **claude CLI** — 選配的 MCP 狀態刷新跑 `claude mcp list`（見下方選配段）；缺就顯示最後快取或留空。
- **PowerShell**（僅 Windows）— 個別欄位輔助查詢用；非 Windows 或缺少時該欄位安全降級。
...
`summary`（頂部摘要）、`dir`、`repo`、`model`、`cost`、`usage`、`quota`、`agents`、`skills`、`crons`（排程狀態，固定佔 agents/skills 欄底兩行）、`memory_mcp`、`edited`、`history`，外加總開關 `enabled`。
