...
1. **理解任務 → 事實層**：讀懂目的與範圍；任務原文含對話指涉（「你掌握的」「你的建議」「這 N 個」「上述」）或指向一份文件說「把沒做完的做完」→ **先把項目逐條展開成任務書的 `## 項目清單`**（編號、內容、驗收方式、來源）。引擎在另一個 session，看不到本對話；工具對不自足的任務會拒絕。事實自己查（讀檔、派 sub-agent），不問使用者。
1b. **決策層（grill）**：依 `goal.grillRounds`（預設 1）問會改變完成條件的分岔——範圍邊界、驗收方式、例外條款、禁止動作、並行共用資源、產出落點——每題附建議答案，不答照建議；本機有 mattpocock 的 grilling skill 就套它的 ❓/➡️ 格式（不呼叫它）。fast-path／plain／使用者說「直接跑」跳過。
2. **propose ＋ 排確認 timer**：
