...
> **鐵律：收到喚醒，第一個 tool call 永遠是「續設下一個 scheduler」——而且要用最短、寫死、不帶長參數的形式**，先把下一棒接力保證住，再去做正事。
...
- 用可用的排程工具設 ~240s 喚醒，prompt 例：「**先續設下一個 scheduler（最短指令）**，再檢查亮色對比修正進度：已改X/5。若上個動作 malformed→重發（同工具連兩次 malformed→換通道）；若進行中→接著改下一個；若 5 個都改完+build 過→取消接力、結束。」（工具名依環境：ScheduleWakeup 或 CronCreate，用前先確認存在）
- 每次喚醒：**第一個 tool call 先續設下一棒**（保證接力鏈不斷），再按核心循環三條判斷走。完成即終止，不留殘留排程。
