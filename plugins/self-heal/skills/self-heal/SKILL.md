...
## 終止機制（kill-switch，使用者喊停即全面靜默）

**旗標檔**：`~/.claude/self-heal.off`（內容不拘，**存在即關**）。這是唯一的終止實體——self-heal 沒有常駐程序，「終止」不是殺 process，而是讓兩層機制都看旗標讓路。

**使用者說「終止 / 停掉 / 關掉 self-heal」時，立刻依序做**：
1. 建旗標檔（`node -e "require('fs').writeFileSync(require('path').join(require('os').homedir(),'.claude','self-heal.off'),'')"` 或等效指令）。
2. 本 session 已排的 self-heal 接力喚醒：能取消就取消；取消不了的不用追——喚醒守衛見旗標即靜默結束（見核心循環第 0 步）。
3. 回報：已終止（含旗標檔路徑），恢復方式一句話帶到。

**使用者說「恢復 / 打開 self-heal」**：刪旗標檔即恢復，下一輪 UserPromptSubmit 起提醒層即回來。

**效果範圍與生效速度**：hook 提醒層（程式強制：`xml-reminder.js` 開頭讀旗標，存在即靜默退出）＋ scheduler 接力層（行為規約：喚醒第 0 步先查旗標）。跨所有專案、立即生效、不動 settings.json、不用重開 session。旗標檢查失敗時 hook 照常提醒（fail-open）。

## 兩層機制

### 第 1 層：事前提醒（本 plugin 的 hook，自動生效）
`hooks/xml-reminder.js` 在每次 UserPromptSubmit 注入 XML 收尾提醒。裝了就生效，無需動作。**開頭先查 kill-switch 旗標，存在即靜默退出（見上節）。**
...
> 不續設的情況只有三種：**使用者喊停 / kill-switch 旗標存在**（順序 0，優先於一切）、正事已**確認完成**（順序 2 的完成分支），或已達重試上限（順序 1 的上限）。
...
每次喚醒依序：
0. **極簡先判**：①kill-switch 旗標檔 `~/.claude/self-heal.off` 存在、或使用者已在對話中喊停 self-heal → **不續設，鏈就此終止**（這優先於一切，包括未完成的正事——使用者喊停就是停）。②正事是否「明顯已完成」或「已達重試上限」？是 → 跳過續設、直接終止/交還（不繞「先設再取消」）。這步只做一眼可判的，別做會 malformed 的重活。
...
> **第一鐵律：一則訊息只送一個工具呼叫（尤其續設 scheduler 一定單獨送）。** 實戰最常見的爆法不是單一工具寫錯，而是「一則訊息塞兩個以上工具呼叫」——其中任一個標籤沒收尾，整批 malformed、連帶把續設也炸掉、接力鏈斷。所以：每則訊息只發一個工具；要連續做多件事，拆成多則訊息逐一發。續設 scheduler、Read、Edit、Bash 全部各自一則，不併送。
