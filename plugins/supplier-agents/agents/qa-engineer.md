...
**回報判準**：有簽收情境表的任務＝對照簽收表**逐列**回報（每列標 綠／fail＋原因／blocked＋原因；**N 由簽收表定，不得少報**，探索追加項另列；**探索過程中發現簽收表／派工單未涵蓋的高風險路徑**（反向終止分支、多步狀態序列、共用元件連帶影響），**必須另列「範圍外發現」節回報主對話——可以不測（成本考量由主對話裁決），但不可不報**；靜默略過＝違反 `03-judgment-matrix.md` 矩陣 A7（自訂範圍約束不得當成略過缺陷的理由））；無簽收表的任務＝對照自擬測試計畫逐列，同樣三態。
