...
- **實際踩的坑**：解釋 git-commit plugin 的流程時讀了它的 SKILL.md、flow.sh、hooks/，每一步都讀對，畫出來的圖卻少了一整條軌。該 plugin 的核心是「三軌**並行**」（A 使用者確認 message ∥ B Codex 審查 ∥ C code-reviewer 審查），而「三軌是哪三軌」這種資訊**不存在於實作檔裡**——它只存在於該專案的模型描述中。產出是一張**每步都對、但講錯這東西在幹嘛**的圖。
- **為什麼比講錯名詞危險**：名詞錯，使用者會發現「這個詞我沒聽過」；關係錯，使用者會**以為自己懂了**——每一步都對，只是整件事的形狀錯了。
- **新增查關係的順序**：`CONTEXT.md`／`CONTEXT-MAP.md` → `README.md` 第一段 → `CHANGELOG.md` 最早那幾則 → 主流程的進入點（看它**同時**啟動什麼、**等待**什麼）。四個地方常常都沒有，沒有就據實說「我是從實作反推的」，不把反推講成事實。
- **三個自檢問句**：① 由哪幾個部分組成？② 哪些同時發生、哪些有先後？③ 少了其中一個會怎樣？答不出來就是還沒讀懂。
- **出處考據**（使用者要求去看 Matt Pocock 的原版）：原版 wait-what 全文只有三行，指定讀 `CONTEXT.md`（多 context 時循 `CONTEXT-MAP.md`），該檔由同 repo 的 `domain-modeling` skill 維護——維護策略是「不批次、當場寫」，並拿程式碼對帳。**但它明訂 `CONTEXT.md` is a glossary and nothing else**，只放詞彙不放關係；關係只在多 context 專案的 `CONTEXT-MAP.md` 才有 Relationships 段。所以照抄原版擋不住這個坑，本節是補上去的。致謝區已註明此差異。⚠️ 我原先誤判成「原版靠 README」，查證後推翻。
- 自檢十九題 → **二十題**。

**同時修正 `flow.js` 的三個缺陷**（第二輪冷啟動 subagent 實測抓到）：

- **`exits[]` 的 `type`／`variant`／`role` 可覆寫，但 header 註解沒寫**。subagent 的原話：「註解叫你分清楚例外與合法分支，但沒告訴你分清楚的欄位在哪」——它第一版因此被迫接受「豁免」被畫成紅色 security 節點，正是註解自己警告的語意反轉。現在註解明列三個欄位，並加一句：只把泳道排除在 `exceptionLanes` 外還不夠，節點與線的預設值仍是紅色 `security`／`error`。
- **泳道順序的控制點沒有任何地方寫**。subagent 改 `spec.lanes` 的 key 順序，圖完全沒變（兩次產出 md5 相同 `74720b2f64e1d845d619baabaecd6cf7`），讀原始碼才知道順序取自 **`exits[]` 陣列裡各條 exit 出現的先後**，`lanes` 的 key 順序被忽略。它說這是三次卡關中唯一沒有錯誤訊息的一次、也最花時間。已寫進 flow.js 註解與 SKILL.md 規則 7。
- **stderr 只列失敗的 edge id，不列成功的**。現在印「垂直線套用 via：4 條（t1, x0, x1, x2）」。
...
- **舊判準作廢**：原本寫「只有視覺佈局／太密／要 mermaid 三種才准升級」，把 archify 當成 ASCII 失敗後的退路，於是流程圖被硬塞進終端機、排得擠又難讀。
- **新判準一句話**：**畫的是「流程」就用 archify；畫的是「關係」或「對照」就留終端機。** 不必先試 ASCII。
- **該用 archify 的題材**（明列）：系統流程圖、多情境與多操作單位、跨部門／跨系統傳遞、審批關卡與退件路徑、一張流程圖就講得完的事、狀態機與時序圖、視覺佈局與 UI。
- **該留終端機的**：兩三個做法的差異（表格）、幾個並列項目與包含關係（縮排樹）、三五個節點的短因果（箭頭圖）、改前改後（上下對照）。
- 兩條反向禁令：不准因為「archify 比較好看」把表格清單也丟給它（兩欄對照開瀏覽器反而慢）；也不准因為「排 ASCII 很麻煩」就升級。
- **掃全文改三處**（不只被點名的那處）：判斷圖整節重寫、archify 五種圖型的對應說明從「三個升級理由」改為題材對照、自檢第 14 題從「有沒有把 ASCII 排得出來的圖丟成 HTML」改為「題材判對了嗎」（雙向：該用 archify 的別硬塞終端機，該留終端機的別開瀏覽器）。
- frontmatter 與 `plugin.json` 的 description 同步改寫，移除「終端機塞不下才升級 HTML（門檻寫死）」。
