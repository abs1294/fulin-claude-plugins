...
> **兩層時間別混淆**（弱模型最常錯的點）：
> - **第 1 層＝任務何時 fire**：quota 重置後（`delaylocal.js` 算出的 cron，`durable:true`）。這是「任務本體」的排程。
> - **第 2 層＝goal 模式的 10 分鐘 propose 確認 timer**：只在 goal 模式、且你 propose 完成條件後才有，是「等使用者確認」的短 timer（`durable:false`），與第 1 層無關。
