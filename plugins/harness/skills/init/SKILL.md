...
Phase 0 前置檢查 ─→ Phase 1 盤點（唯讀） ─→ Phase 2 攤開核對 ─→ Phase 3 訪談（一次一題，Q1~Q7）
                                                                          │
              收尾回報 ←─ Phase 5 驗收（靜態五條＋冷啟探針四項） ←─ Phase 4 生成（五層）
```
...
```
本專案的 commit 流程需要 Codex CLI 當第二審查軌（與 code-reviewer 不同源的審查者）。
偵測結果：`codex --version` → <實際輸出，例：command not found>

請先安裝並登入：
  1. npm install -g @openai/codex        （需要 Node.js）
  2. codex login                          （用你的 OpenAI 帳號）
  3. codex --version                      （確認有版本號）
裝好後重跑 /harness:init，Phase 0 會重新檢查四項。

為什麼不直接略過：少了 Codex，commit 前的審查只剩同一家模型審自己寫的東西，
盲區完全同構——這正是 harness 要防的「驗證器與施作器同源」。
```
...
結尾一句：「以上事實有錯請直接指出；沒有的話我開始逐題訪談（共 N 題）。」使用者糾正的事實回 Phase 1 重查確認後更新表格。**無人值守（headless）時**：照推導預設繼續，所有推導值在收尾回報列為「已代決」供事後否決。
...
正本＝`references/hook-catalog.md`（來源專案 20 支 hook＋3 個 plugin 自帶閘，逐支拆成「通用形狀＋觸發條件＋專案參數」，分 A 必裝／B 盤點觸發／C 工作法觸發／D 由 plugin 提供／E 長出來才裝五類）。**沒有數量上限**——裝幾支由這個專案的事實決定。

推導（事實判定，不問使用者）：

1. 形狀目錄逐列判觸發條件，用 Phase 1 十一項與 Q1–Q4 的答案。
2. **B 類的每一個風險都要落成一條規則**：Phase 1 第 8 項的危險動作候選（Q2 勾選的）、第 10 項的執行期風險事實，各自變成 `guard-risky-command.js` 或 `guard-test-preconditions.js` 的一條規則。**熔斷清單上的項目只寫進文字、沒有對應規則＝推導不完整**——只能靠人判斷、做不成樣式比對的，要在 Q5 與收尾回報逐項說明為什麼停在文字。
3. D 類不複製，列進安裝指引；E 類不裝，寫進 05 §6 的「可升格機械閘」清單。

Q5 攤給使用者的格式（每支一列）：

```
| hook | 類別 | 擋什麼 | 為什麼這個專案需要（證據） |
| check-agent-model | A | 派專案 agent 沒帶 model | Q1 建了 4 支 agent |
| guard-risky-command：db-admin-login | B | 以 postgres 帳號連資料庫 | api/.env.example:3 連線字串用 postgres |
| guard-test-preconditions：mail-sink | B | 跑測試時 SMTP 沒指向本機收信器 | api/src/server.js:5 用 nodemailer |
...
不裝的：check-wt-hacks-on-start（沒有多工作樹）、guard-test-asset-hygiene（還沒有測試稽核工具，屬 E 類）
```
...
1. **落地清單**：五層逐檔列出路徑；關鍵改編決策（裁切了什麼、讓位了什麼、代決了什麼——無人值守時 Phase 2 的推導預設全列為「已代決」）。
2. **驗收證據**：靜態五條逐條結果＋冷啟探針四項的原始輸出。
3. **你已經有的**：
   - 一套開發流程骨架：pipeline＋配套三條、40 條判斷矩陣（停損／完成判準／熔斷）、派工模板、知識協議
   - <N> 支會真的擋人的 hook（依形狀目錄分類列名；B 類引擎列出每條規則對應的風險）、<N> 支 agent（列名）
   - 形狀目錄裡**沒裝的**逐列列出與理由（不適用／使用者取消／由 plugin 提供待安裝／E 類待長出稽核工具）
   - 三份知識容器的空殼與收錄原則
4. **你還沒有的**（照實列，不要美化）：
   - **踩坑知識 0 條**——memory、CONTEXT、FLOWS、PROJECT 都是空的（只有示範條目）。成熟的 harness 靠的是數十到上百條「這裡曾經出過什麼事」，那些只能從本專案自己的工作裡長出來。
   - **回歸測試 0 支**（init 不寫測試）<若 Phase 1 盤點到既有測試，寫「既有測試 N 支，未經 harness 流程檢視」>
   - 模組覆蓋登記、專案專屬的稽核工具、事故型條款——全部沒有。
   - 目前的制度檔只是骨架：規則的數字與範例都是通用的，還沒被本專案的真實事故校正過。
5. **接下來怎麼長**：
   - 每次踩坑 → 照 05 §2 寫 memory；同類坑第 2 次 → 照 05 §6 提議升格成制度條款（該配機械閘就一起配）
   - 每次 commit → 沉澱閘逼你回答詞／鏈／QA／代號四題，容器從這裡慢慢長
   - 每次行為類改動 → QA codify 一支可重跑測試，回歸網從這裡長
   - 30 天 → 照 05 §5 健檢（含 hook dry-run 兩個方向）
6. **生效範圍限制**：hooks 只在設定所在層級、在該目錄開 session 時生效；子 repo 內開 session 時 CLAUDE.md 仍會被讀到（祖先目錄），但 workspace 根的 hooks 不會。
7. **盤點發現的目標專案自身問題**（文件漂移、缺測試基礎、敏感物未擋）誠實回報，不靜默。
