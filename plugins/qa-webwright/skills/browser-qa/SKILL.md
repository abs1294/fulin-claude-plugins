...
### 增量分流（reuse-first，設計前先做）

**設計任何新 TC 之前，先盤點既有測試資產**：專案已有 codified 測試套件時（含索引／情境正本，如 CATALOG、COVERAGE 這類檔案），先查「這次變更落在哪個模組、既有案例的導航與斷言點能不能到達」。二選一表態後才進設計：

- **reuse**：變更落在既有案例的 seam 內（改欄位／驗證規則／顯示邏輯）→ **直接改既有 runner**：加 assert／參數化，只跑受影響案例＋同資料夾回歸——不開瀏覽器探索、不寫新 TC。既有腳本已內建登入、導航、seed，邊際成本只剩斷言，且斷言永久留在回歸網。
- **新 TC**：無既有覆蓋／新畫面／新互動、或需視覺判斷（排版、loading、console 紅字）→ 附一句理由（為何既有案例接不住），走完整 Phase 1→2。

一句話判準：**既有腳本能到達並斷言的行為，不重新探索**。新功能的主動探索情境紀律不因此豁免——它管新功能的未知未知，本節管小改的增量。
...
- **可識別**：測試造的業務資料（單據、名稱、標題類欄位）一律帶專案約定的測試前綴（無約定時用 `E2E-`）——含瀏覽器探索階段造的資料，殘留才能被機械定位。
- **自種自清**：造資料的 runner 必配 fixture teardown（`yield` 後清理，失敗也會執行）。「跑完整生命週期到終態」是首選（終態路徑本身有測試價值），但它扛不住測試中途失敗——teardown 是兜底，兩者並用。
- **收尾清掃**：大批執行結束後，用專案的殘留清掃工具（或按前綴查詢）確認無測試殘留，才算收工。
- **只刪自己造的**：teardown 用 `SCOPE_IDENTITY`／唯一前綴鎖定自己造的那幾筆，**不要 UPDATE 回原值**——那會把別人的資料改壞。

### ⭐ 五條硬規則：測試不得依賴環境既有資料（寫 assert 前必讀）

判準一句話：**把業務資料清空、只留 schema 與字典，這支測試仍該能跑通**。
...
2. **預擬 codify 草稿（draft-first，不先開瀏覽器）**：直接把每個 critical point 落成 `tests/e2e/test_<feature>.py` 裡
   ...
   **草稿同時要滿足上方五條硬規則**：前提資料走產品入口自造（非寫死既有 Id）、期望值查來源推導（非字面數字）、
   定位錨語系無關（非中文顯示字）、沒資料時自造（非 skip）、開跑前驗 `response.url` 確認打對後端。
   斷言規範與五條硬規則的完整判準見 `methodology/critical-points.md`。
