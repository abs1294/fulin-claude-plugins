...
migrate_legacy_catalog() {
  ...
  # 逐列讀舊檔資料列，函式（第 2 欄）不在新檔就 append
  local line func
  while IFS= read -r line; do
    ...
    if ! awk -F'|' 'NR>0{gsub(/^ +| +$/,"",$3); if($3==f) found=1} END{exit !found}' f="$func" "$CATALOG_FILE"; then
      printf '%s\n' "$line" >> "$merged"
    fi
  done < "$legacy"
  mv "$merged" "$CATALOG_FILE"
  mv "$legacy" "$legacy.migrated-$today"
  echo "[qa-flow] 已合併舊版 catalog 資料列 → $CATALOG_FILE；舊檔改名 $legacy.migrated-$today（可人工確認後刪）" >&2
}
...
# Windows 常無 `pytest` 命令但有 `python -m pytest`，自動挑一個免使用者建 shim。
detect_pytest() {
  PYTEST_CMD=""
...
cmd_bootstrap() {
  ...
  # --- 環境自檢：這些缺了會讓落地/他律靜默失效，早點暴露（尤其分享給同事的機器）---
  echo "--- 環境自檢 ---"
  if command -v node >/dev/null 2>&1; then
    echo "node： $(node --version 2>/dev/null)"
  else
    echo "node： ⚠️ 找不到——**Stop hook 落地強制在本機不會生效**（hook 靠 node 執行，harness 會靜默跳過）。" >&2
    echo "       請安裝 Node.js，否則 AI 跳過落地時不會被擋。" >&2
  fi
  detect_pytest
  if [ -n "$PYTEST_CMD" ]; then
    echo "pytest： $PYTEST_CMD（可用）"
  else
    echo "pytest： ⚠️ 找不到（試過 pytest / python -m pytest / python3 / py）——run 階段會報錯。" >&2
    echo "       greenfield 請於 scaffold 後執行：pip install pytest-playwright && playwright install chromium" >&2
  fi
  echo ""
...
EOF
    ...
    echo "      → qa-flow.sh run $feature tests/e2e/test_${feature_id}.py（date 可省略=今天）"
...
cmd_run() {
  local feature="${1:-}"
  local test_file="${2:-}"
  # date 是確定性資訊，交給 AI 傳反而是失誤點（曾有 AI 傳字面 "<date>" 佔位符）→ 預設今天。
  local date="${3:-$(date +%F)}"
  ...
  # 守門員（symlink 逃逸偵測）：resolve_test_file 已在**字串層**鎖死落點 = WORKSPACE_DIR/tests/e2e/<檔名>
  # （拒絕絕對路徑 / '..' / tests/e2e 外），故一般路徑逃逸已擋。canonical 比對唯一要多抓的是
  # 「測試檔本身是 symlink，字串合法但實體指向 tests/e2e 外」。
  #
  ...
  # canonical 解析失敗（任一邊為空）→ 字串層已鎖定，放行不誤擋。
  ...
  # 偵測 pytest 怎麼跑（Windows 常無 pytest 命令但有 python -m pytest，免自建 shim）。
  detect_pytest
...
# ------------------------------------------------------------
# Command: audit [--fix]
#   核對 catalog.md 的「對應測試函式」欄 vs tests/e2e/ 內實際存在的 test 函式，
#   揪出孤兒列（函式已消失 = catalog 漂移成「看起來權威的錯誤索引」）。
#   --fix：把孤兒列的覆蓋狀態改成 ❌未覆蓋（備註函式已不存在），不刪列（保留應測情境語意）。
#   回傳：有孤兒且未 --fix → exit 3（供呼叫端判別）；乾淨或已修 → exit 0。
# ------------------------------------------------------------
...
cmd_audit() {
  ...
  while IFS= read -r line; do
    ...
    case "$line" in *白話業務情境*) continue ;; esac # 跳表頭
    case "$line" in \|[[:space:]-]*[-][[:space:]-]*\|*) : ;; esac
    # 分隔線（| --- | --- |）跳過
    printf '%s' "$line" | grep -qE '^\|[[:space:]-]+\|' && continue
    # 取第 2 欄（awk 以 | 分欄，欄位從 $3 起因 $1 為行首空）
    func="$(printf '%s' "$line" | awk -F'|' '{gsub(/^ +| +$/,"",$3); print $3}')"
    [ -z "$func" ] && continue
    case "$func" in "—"|"-") continue ;; esac       # 未覆蓋列（無函式）不算孤兒
    if ! printf '%s\n' "$funcs_present" | grep -qxF "$func"; then
      orphans+=("$func")
    fi
  done < "$CATALOG_FILE"
  ...
  # --fix：把孤兒列的覆蓋狀態欄改為 ❌未覆蓋、模組欄後補備註（不刪列）
  local tmp orph_list
  tmp="$(mktemp)"
  orph_list="$(printf '%s\n' "${orphans[@]}")"
  awk -v orphans="$orph_list" '
    BEGIN { n=split(orphans, arr, "\n"); for(i=1;i<=n;i++) if(arr[i]!="") isorph[arr[i]]=1 }
    /^\|/ && $0 !~ /白話業務情境/ && $0 !~ /^\|[- ]+\|/ {
      nf=split($0, c, "|")
      f=c[3]; gsub(/^ +| +$/,"",f)
      if (f in isorph) {
...
Commands:
  ...
  run      <feature> <test-file> [date]  grep 驗證 test 函式存在（防假綠燈）→ pytest --junitxml 出報告
                                         （date 省略=今天；自動偵測 pytest / python -m pytest）
  catalog  <情境> <函式> <狀態> <模組>   機械回填 tests/e2e/catalog.md 總表（以函式為主鍵 update/append）
  audit    [--fix]                        核對 catalog 對應函式 vs 實際 test 函式，揪孤兒列；--fix 標成 ❌未覆蓋
