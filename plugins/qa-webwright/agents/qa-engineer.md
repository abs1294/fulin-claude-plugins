...
**必須完整輸出以下四段，順序固定，每段都要有——禁止只給摘要 / 重點版 / 「精簡起見省略」。**
...
| 維度 | 值 / 分支 | 對應 TC | 備註 |
|------|----------|---------|------|
（窮舉所有狀態維度，每格至少一條 TC；任一格沒 TC = 不完整）

### 2. 測試案例
（每條 TC 用 `methodology/test-plan-design.md` §3 的完整格式：TC-編號 / 功能 / 前置條件 /
  操作步驟+【證據】+預期結果 / 需檢查項目——逐條全列，不可只列代表性幾條）

### 3. 需求 ↔ TC 對照表
| 需求 / 驗收條件 | 對應 TC | 備註（不測則寫「不測 + 理由」）|
|----------------|---------|------|

### 4. 紅隊漏測複查（大型功能必做，小改可寫「略過 + 理由」）
（獨立視角只讀矩陣+diff+驗收條件，列疑似漏測並補 TC 一次）
```

每條 TC 的每個預期結果都要標好「這個 critical point 用哪個結構化證據驗證」
（API 業務碼 / DOM 讀回值 / 來源 readback），讓主 Agent 直接落成 pytest 的一行 assert。

> **禁止**：因功能簡單就只給幾條代表 TC、或只回摘要說「大致要測 X/Y/Z」。四段一律完整輸出。
