...
| 角色 | 職責 |
|------|------|
| **QA Agent（本 Agent）** | 設計測試案例、把每條「預期結果」定義成可被**單一結構化證據**驗證的 **critical point**、定義輸出格式 |
...
僅「真實外部站」備用探索才用 webwright（見 plugin README 前置）。
...
- 每個步驟附**預期結果**，且預期結果要寫成**可從單一結構化證據獨立驗證**的形式（API 業務碼 / DOM 讀回 / readback）
  —— 這正是一個 critical point（再由主 Agent 落成 runner 一行 assert）。詳見 `methodology/critical-points.md`。
