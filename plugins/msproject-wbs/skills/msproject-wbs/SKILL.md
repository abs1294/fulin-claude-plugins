...
複製 `scripts/generate-wbs.mjs`，**只改頂部的 `MODULES` 陣列**與 `OUT`/`PROJECT_NAME`/`RESOURCES`/`TODAY`（用週次落位才需再設 `START`/`W1_MON`）。腳本內的排程演算法、MSPDI 框架、雷區處理**不要動**——那是本 skill 的核心價值。範本頂部附完整示範資料（同時展示三層 / 兩種排程 / 里程碑 / 貫穿全期），照著改即可。

工項物件格式（見腳本內註解，依需要挑欄位）：
```
基本：   { id:'唯一id', name:'工項名(純中文,不放代號)', res:'資源key' }
排程三選一：
  依賴鏈： d:工期天數, dep:'前置工項id'(選填)  ← 簡單專案，腳本自動依資源時間線+依賴往後排
  週次區間：w:[起週,迄週]                      ← W(a)週一→W(b)週五，inclusive
  天粒度： pd:[絕對週次, 起日, 迄日]            ← 週內第幾天 0=一…4=五
  前期週次：pre:[起,迄]                         ← 落在 W1 之前的前期(以 START 為基準)
層級/標記：
  sum:true  此為 L2 主條目，其後緊接的一般工項自動歸它當 L3 子項（要三層時用）
  ms:true   里程碑（菱形，單日）
  top:true  貫穿全期/獨立 L2 工項（不當某條目的子項）
```
> **何時用哪種排程**：時程**並行重疊**（例 系統二 W7–W10 與 系統三 W7–W8 同時跑）→ 必用週次落位（`w`/`pd`），依賴鏈會把重疊硬拆成串行、日期就對不上。單純線性專案用依賴鏈最省事。兩種可在同一份 MODULES 混用。
> **兩層 vs 三層**：只給模組(L1)+工項即為兩層；要三層就把中層條目標 `sum:true`，其後的工項自動變它的 L3 子項。
